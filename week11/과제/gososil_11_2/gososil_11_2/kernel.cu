...
__global__ void Fibonacci_Kernel(int *x, int* y)
{
    ...
    /*Todo*/
    int i, j;
    float sqrt5 = sqrtf(5.0f);
    float x0, x1, tmp0, tmp1;
    x0 = (1.0f + sqrt5) / 2.0f;
    x1 = (1.0f - sqrt5) / 2.0f;
    tmp0 = tmp1 = 1.0f;
    for (j = 0; j < x[tid]; j++) {
    ...
    y[tid] = (int)((tmp0 - tmp1) / sqrt5 + 0.5f);
}
...
cudaError_t Fibonacci_GPU(int *h_Fibo_n, int *h_Fibo_v, int *h_Fibo_v_GPU)
{
    ...
    size_t size;
    size = N * sizeof(int);
    CUDA_CALL(cudaMalloc(&Fibo_n, size))
    CUDA_CALL(cudaMemcpy(Fibo_n, h_Fibo_n, size, cudaMemcpyHostToDevice))
    CUDA_CALL(cudaMalloc(&Fibo_v, size))
    dim3 dimBlock(BLOCK_WIDTH, BLOCK_HEIGHT);
    dim3 dimGrid((1 << 13) / BLOCK_WIDTH, (1 << 13) / BLOCK_HEIGHT);
    CHECK_TIME_START_GPU()
    Fibonacci_Kernel << < dimGrid, dimBlock >> > (Fibo_n, Fibo_v);
    CHECK_TIME_END_GPU(device_time)
    CUDA_CALL(cudaGetLastError())
    CUDA_CALL(cudaDeviceSynchronize())
    CUDA_CALL(cudaMemcpy(h_Fibo_v_GPU, Fibo_v, size, cudaMemcpyDeviceToHost))
    cudaFree(Fibo_n);
    cudaFree(Fibo_v);
    /*Todo end*/
    CHECK_TIME_DEST_GPU();
