...
void read_bin_file() {
    ...
    FILE* fp = fopen("Cuda HW3 input.bin", "rb");
...
void init_bin_file(IN int n, IN int nf) {
    ...
    FILE* fp = fopen("Cuda HW3 input.bin", "wb");
...
int main()
{
    ...
    printf("***N_SIZE : %d NF_SIZE : %d BLOCK_SIZE : %d\n", N_SIZE, NF_SIZE, BLOCK_SIZE);
...
cudaError_t Sum_n_elements_GPU(IN int* p_ArrayElements, OUT int* p_SumOfElements_GPU, int Nf) {
    ...
    size_t size;
    size = N * sizeof(int);
    CUDA_CALL(cudaMalloc(&arr, size))
        CUDA_CALL(cudaMemcpy(arr, p_ArrayElements, size, cudaMemcpyHostToDevice))
        CUDA_CALL(cudaMalloc(&sum, size))
        dim3 dimBlock(BLOCK_WIDTH, BLOCK_HEIGHT);
    dim3 dimGrid((1 << 13) / BLOCK_WIDTH, (1 << 13) / BLOCK_HEIGHT);
    CHECK_TIME_START_GPU()
        Sum_n_elements_Kernel << < dimGrid, dimBlock >> > (arr, sum, N, Nf);
    CHECK_TIME_END_GPU(device_time)
        CUDA_CALL(cudaGetLastError())
        CUDA_CALL(cudaDeviceSynchronize())
        CUDA_CALL(cudaMemcpy(p_SumOfElements_GPU, sum, size, cudaMemcpyDeviceToHost))
        cudaFree(arr);
    cudaFree(sum);
    ////
    CHECK_TIME_DEST_GPU();
